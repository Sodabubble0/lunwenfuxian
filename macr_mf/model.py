...
class BPRMF:
    def __init__(self, args, data_config):
        ...
        # two branch
        self.w = tf.compat.v1.Variable(self.initializer([self.emb_dim,1]), name = 'item_branch')
        self.w_user = tf.compat.v1.Variable(self.initializer([self.emb_dim,1]), name = 'user_branch')
    ...
    def init_weights(self):
        weights = dict()
        self.initializer = tf.compat.v1.initializers.glorot_uniform()
        initializer = self.initializer
        with tf.compat.v1.variable_scope('parameter'):
            weights['user_embedding'] = tf.compat.v1.Variable(initializer([self.n_users, self.emb_dim]), name = 'user_embedding')
            weights['item_embedding'] = tf.compat.v1.Variable(initializer([self.n_items, self.emb_dim]), name = 'item_embedding')
            weights['user_rand_embedding'] = tf.compat.v1.Variable(initializer([self.n_users, self.emb_dim]), name = 'user_rand_embedding', trainable = False)
            weights['item_rand_embedding'] = tf.compat.v1.Variable(initializer([self.n_items, self.emb_dim]), name = 'item_rand_embedding', trainable = False)
        with tf.compat.v1.variable_scope('const_embedding'):
            self.rubi_c = tf.compat.v1.Variable(tf.zeros([1]), name = 'rubi_c')
            weights['c'] = tf.compat.v1.Variable(tf.zeros([1, self.emb_dim]), name = 'c')
        
        weights['user_c'] = tf.compat.v1.Variable(tf.zeros([self.n_users, 1]), name = 'user_c_v')
