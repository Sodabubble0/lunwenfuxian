...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        self.beta = args.beta
        self.rubi_c = tf.compat.v1.Variable(tf.zeros([1]), name = 'rubi_c')
        ...
        *********************************************************
        ...
        self.opt_two_bce1 = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss_two_bce1)
        ...
        self.opt_two_bce2 = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss_two_bce2)
    
    
    def create_model_str(self):
        # 区分普通LightGCN模型和MACR-LightGCN模型的日志路径
        if args.test == 'normal':
            model_name = 'normal_lightgcn'
        else:
            model_name = 'macr_lightgcn_' + args.test
            
        log_dir = '/' + model_name + '/layers_' + str(self.n_layers) + '/dim_' + str(self.emb_dim)
    ...
    def _init_weights(self):
        ...
        all_weights["constant_embedding"] = tf.compat.v1.Variable(tf.ones([1, self.emb_dim]), name='constant_embedding')
        if self.pretrain_data is None:
            all_weights['user_embedding'] = tf.compat.v1.Variable(initializer([self.n_users, self.emb_dim]), name='user_embedding')
            all_weights['item_embedding'] = tf.compat.v1.Variable(initializer([self.n_items, self.emb_dim]), name='item_embedding')
            print('using xavier initialization')
        else:
            all_weights['user_embedding'] = tf.compat.v1.Variable(initial_value=self.pretrain_data['user_embed'], trainable=True,
                                                        name='user_embedding', dtype=tf.float32)
            all_weights['item_embedding'] = tf.compat.v1.Variable(initial_value=self.pretrain_data['item_embed'], trainable=True,
                                                        name='item_embedding', dtype=tf.float32)
            print('using pretrained initialization')

        self.w = tf.compat.v1.Variable(initializer([self.emb_dim, 1]), name = 'item_branch')
        self.w_user = tf.compat.v1.Variable(initializer([self.emb_dim, 1]), name = 'user_branch')
            
        self.weight_size_list = [self.emb_dim] + self.weight_size
        
        for k in range(self.n_layers):
            all_weights['W_gc_%d' %k] = tf.compat.v1.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k+1]]), name='W_gc_%d' % k)
            all_weights['b_gc_%d' %k] = tf.compat.v1.Variable(
                initializer([1, self.weight_size_list[k+1]]), name='b_gc_%d' % k)

            all_weights['W_bi_%d' % k] = tf.compat.v1.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k + 1]]), name='W_bi_%d' % k)
            all_weights['b_bi_%d' % k] = tf.compat.v1.Variable(
                initializer([1, self.weight_size_list[k + 1]]), name='b_bi_%d' % k)

            all_weights['W_mlp_%d' % k] = tf.compat.v1.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k+1]]), name='W_mlp_%d' % k)
            all_weights['b_mlp_%d' % k] = tf.compat.v1.Variable(
...
class sample_thread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            print(f"Error in sample_thread: {e}")
            # 如果出现异常，使用一个空样本
            dummy_size = args.batch_size
            self.data = ([0] * dummy_size, [0] * dummy_size, [0] * dummy_size)
...
class sample_thread_test(threading.Thread):
    ...
    def run(self):
        try:
            ...
            print(f"Error in sample_thread_test: {e}")
            # 如果出现异常，使用一个空样本
            user_list = list(data_generator.test_set.keys())
            if user_list:
                # 确保至少有一个可用的用户
                user_sample = [user_list[0]] * min(args.batch_size, len(user_list))
                user_sample = user_sample[:args.batch_size]
                # 创建空样本数据
                pos_items = [0] * len(user_sample)
                neg_items = [0] * len(user_sample)
                self.data = (user_sample, pos_items, neg_items)
            else:
                # 完全找不到用户时的备选方案
                dummy_size = args.batch_size
                self.data = ([0] * dummy_size, [0] * dummy_size, [0] * dummy_size)
    ...
    def run(self):
        ...
        if len(gpus):
            with tf.device(gpus[-1]):
                ...
                self.data = self.sess.run(sess_list,
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        ...
        # 区分普通LightGCN模型和MACR-LightGCN模型的保存路径
        if args.test == 'normal':
            model_type = 'normal_lightgcn'
        else:
            model_type = 'macr_lightgcn_' + args.test
        
        # 构建保存路径，包含模型类型和数据集信息
        weights_save_path = '%sweights/%s/%s/%s/l%s_r%s' % (args.weights_path, args.dataset, model_type, layer,
                                                            str(args.lr), '-'.join([str(r) for r in eval(args.regs)]))
        ensureDir(weights_save_path)
        save_saver = tf.compat.v1.train.Saver(max_to_keep=20000)
    ...
    *********************************************************
    ...
    if args.pretrain == 1:
        # MACR
        # 根据数据集和模型类型构建预训练模型路径
        model_type = 'macr_lightgcn_' + args.test if args.test != 'normal' else 'normal_lightgcn'
        layer = '-'.join([str(l) for l in eval(args.layer_size)])
        model_path = '%sweights/%s/%s/%s/l%s_r%s' % (args.weights_path, args.dataset, model_type, layer,
                                                  str(args.lr), '-'.join([str(r) for r in eval(args.regs)]))
        model_file = model_path + '/weights_{}-{}'.format(args.saveID, args.pretrain_epoch)
        print("Loading pretrained model from: ", model_file)
